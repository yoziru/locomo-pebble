...
void update_text(void) {
  ...
  if (!train_departure_is_ok(departures[pos]) && (bgcolor_changed == 0)) {
    ...
    bgcolor_changed = 1;
  } else if (train_departure_is_ok(departures[pos]) && bgcolor_changed == 1) {
...
static void handle_empty(char* data) {
  destroy_departures();

  data_processor_init(data, '|');
  text_layer_set_text(text_layer_status, "EMPTY");
  text_layer_set_text(text_layer_departure_arrival, data);
}
...
void refresh_data(void) {
  ...
  mqueue_add("TRAIN", "DEPARTURES", "update");
...
void switch_data(void) {
  ...
  mqueue_add("TRAIN", "DEPARTURES", "switch");
